...
async function httpGetPanoramaScene(req, res) {
     ...
     const hotel = req.url.split('/')[2].split('?')[1].split('&')[0].split('=')[1];
     const appType = req.url.split('/')[2].split('?')[1].split('&')[1].split('=')[1];
     const appId = req.url.split('/')[2].split('?')[1].split('&')[2].split('=')[1];
